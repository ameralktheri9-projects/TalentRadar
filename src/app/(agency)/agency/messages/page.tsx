...
interface Thread {
  ...
  messages: Array<{ body: string; readByAgencyAt: string | null }>;
}
...
export default async function AgencyMessagesPage() {
  ...
  return (
    <div dir="rtl">
      <Header title="الرسائل" subtitle="محادثاتك مع الشركات" />
      <div className="p-6 max-w-3xl mx-auto space-y-3">
        {threads.length === 0 ? (
          ...
          threads.map((thread) => {
            ...
            const unread = lastMsg && !lastMsg.readByAgencyAt;
            return (
              <Link
                key={thread.id}
                href={`/agency/messages/${thread.id}`}
              ...
              >
                ...
                <div className="text-sm text-gray-500 mb-1">{thread.proposal.job_request.company?.name_ar}</div>
                {lastMsg && (
