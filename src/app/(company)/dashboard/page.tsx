...
export default async function CompanyDashboardPage() {
  ...
  return (
    <div>
      <Header title="لوحة تحكم الشركة" subtitle="مرحباً بك في منصة صيد المواهب" />

      <div className="p-6 space-y-6">
        {/* Stats Grid */}
        <div className="grid grid-cols-4 gap-4">
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="text-sm text-gray-500 mb-1">طلبات التوظيف المفتوحة</div>
            <div className="text-2xl font-bold text-gray-800">{openJobRequests}</div>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="text-sm text-gray-500 mb-1">العروض النشطة</div>
            <div className="text-2xl font-bold text-gray-800">{activeProposals}</div>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="text-sm text-gray-500 mb-1">فواتير معلقة</div>
            <div className="text-2xl font-bold text-yellow-600">{pendingInvoices.length}</div>
            <div className="text-xs text-gray-400 mt-1">{pendingInvoiceTotal.toFixed(0)} ر.س</div>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="text-sm text-gray-500 mb-1">مرشحون للمراجعة</div>
